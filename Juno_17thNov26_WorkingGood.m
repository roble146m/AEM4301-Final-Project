function [rsc,vsc,finalDate] = Juno_17thNov26_WorkingGood(initialDate)
...
%% Initialize
   ...
   maxDays= 1829;         % Number of days to follow the spaceraft
   dsmDay = 255;         % This is the dayCount where DSM is executed
                         % Can be refined (near apoapsis)
   fbday1= 786;         % This will be the day of flyby. Can be refined.
...
%% Launch Maneuver
  ...
  Vsc=Vsc - 0.745*Vsc/norm(Vsc); % Decrease velocity. Experiment with magnitude of Delta V here.
...
%
  Rflyby = [-49184566.9534125	138092706.458376	0];
  VscFlyby = [-35.2300064293936	-3.39392745272867	0];
  VpFlyby = [-28.5433677285647	-10.1121229387973	0];
  % Calculate the velocity after the flyby
  [VscFlyby,DeltaMin]=flyby(VpFlyby,VscFlyby,12499,398600,6378,1);
  DeltaMin
  %Calculate the orbital elements for the spacecraft after the flyby
  [h,a,e,w,E0]=scElements(Rflyby,VscFlyby);
